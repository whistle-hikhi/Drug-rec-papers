# Drug-rec-papers

A working literature collection on **drug / medication recommendation systems**, organized for ongoing PhD reading. Covers four sub-areas: EHR-based recommendation, drug-drug interaction (DDI) aware recommendation, knowledge graph / GNN methods, and LLM / generative approaches.

Each paper is tracked with year, venue, a one-line summary of its core idea, and a link. Full citations live in [`references.bib`](references.bib). Cross-cutting themes and open questions are in [`NOTES.md`](NOTES.md).

## How this is organized

```
papers/
  01-ehr-based/          patient history -> medication set/sequence prediction
  02-ddi-aware/           interaction-safety-constrained recommendation & DDI prediction
  03-knowledge-graph-gnn/ KG- and GNN-based drug/medicine modeling
  04-llm-generative/      LLMs for medication recommendation & clinical decision support
  05-fairness-bias/       fairness/bias evidence & methodology, cross-cutting across 01-04
  06-datasets-beyond-mimic/ non-MIMIC evaluation datasets (eICU, CPRD, NHIRD, private/non-US EHR)
  07-human-factors/       clinician trust, alert fatigue, automation bias, adoption/implementation science
datasets/                catalog of actual datasets/databases (EHR, KGs, DDI, molecular) used across the above
references.bib            consolidated BibTeX for everything below
NOTES.md                  synthesis: trends, tensions, gaps across the four areas
```

Note the categories overlap heavily in practice — e.g. GAMENet/SafeDrug are EHR-based models that are *also* DDI-aware, and several LLM papers use KG grounding. A paper is filed under its primary contribution; cross-references are noted inline.

## Reading starting points (surveys)

Start here before diving into individual models — these give the lay of the land for each sub-area:

| Area | Survey | Venue/Year |
|---|---|---|
| EHR-based medication rec | [Deep Learning for Medication Recommendation: A Systematic Survey](https://direct.mit.edu/dint/article/5/2/303/114891/Deep-Learning-for-Medication-Recommendation-A) | Data Intelligence (MIT Press), 2023 |
| DDI prediction | [Deep learning for drug-drug interaction prediction: A comprehensive review](https://onlinelibrary.wiley.com/doi/full/10.1002/qub2.32) | Quantitative Biology, 2024 |
| KG for drug repurposing | [Knowledge Graphs for drug repurposing: a review of databases and methods](https://academic.oup.com/bib/article/25/6/bbae461/7774899) | Briefings in Bioinformatics, 2024 |
| LLMs for clinical reasoning | [Aligning Clinical Needs and AI Capabilities: A Survey on LLMs for Medical Reasoning](https://arxiv.org/pdf/2607.07761) | arXiv, 2026 |
| Fairness/bias in clinical AI | [Dissecting Racial Bias in an Algorithm Used to Manage the Health of Populations](https://www.science.org/doi/10.1126/science.aax2342) — landmark, not a survey but the reference point everything else cites | Science, 2019 |

## Status

Seeded 2026-08-19 via targeted search across the four focus areas (~55 papers), then extended three times the same day: to fill out `04-llm-generative`'s architecture side (agentic/multi-agent, RAG, fine-tuning/alignment, KG-LLM fusion); to add `05-fairness-bias` (~29 papers) after the initial pass surfaced no fairness/bias coverage at all; and to add `06-datasets-beyond-mimic` (~9 papers/resources) after noting every `01-ehr-based` model was benchmarked on MIMIC alone.

Refreshed 2026-09-08, twice. First pass: a general sweep for new/recent papers across all six areas (~26 additions, mostly 2025-2026), headlined by **RAREMed and DMRNet**, the first two papers found evaluating fairness directly on the medication-recommendation task itself (drug-frequency, not yet demographic), plus a title correction (arXiv:2510.21084 is "MediRec," not "Chinese Discharge Drug Recommendation..." as previously listed). Second pass: added `07-human-factors` (~26 papers) — clinician trust, automation bias, alert fatigue/override behavior, explainability/acceptance, and adoption/implementation science. Unlike the earlier gaps, this one turned out *not* to be a search artifact: it's a mature, decades-old sub-field, just one that studies today's rule-based alert systems rather than ML combinatorial recommenders — so the model-side and human-factors literatures still don't meet at the same object of study.

Also added on request: "Patients-like-me" (arXiv:2608.04193, Aug 2026) — a variational LM-GNN framework evaluated on MIMIC across four tasks including medication recommendation, explaining predictions via retrieved similar patients — filed in `01-ehr-based`.

Added 2026-09-21: [`datasets/`](datasets/README.md), a catalog of the actual datasets/databases behind this literature (MIMIC-III/IV, eICU, CPRD, NHIRD, DrugBank, PrimeKG, Hetionet, SIDER, BIOSNAP ChCh-Miner, DDInter, TWOSIDES/OFFSIDES, PubChem/ChEMBL), with access requirements and links — distinct from `06-datasets-beyond-mimic`, which tracks papers that use non-MIMIC data rather than the datasets themselves.

Then a "find new top papers" sweep (6 additions), headlined by **TxGNN** (`03-knowledge-graph-gnn`) — Huang et al., *Nature Medicine* 2024, a graph foundation model for zero-shot drug repurposing across 17,080 diseases from the Zitnik Lab; by publication venue, lab, and downstream coverage it's the single most prominent paper in this whole collection. Also added: DMNC (KDD 2018), a foundational EHR-based baseline that GAMENet/SafeDrug/MICRON all benchmark against but that had never been added to this table itself; two 2025 papers opening a new "confidence calibration" axis in `01-ehr-based` (KDD 2025, SAFER) distinct from the usual accuracy/DDI-safety tradeoff; RES-MR (SIGIR 2026, `04-llm-generative`), which argues for *personalized* rather than fixed patient-agnostic safety constraints; and "When AI Gets It Wrong" (`07-human-factors`), whose central finding — missed-risk false negatives are far more dangerous than false positives — is a concrete asymmetry worth citing wherever a safety-recall/precision tradeoff is discussed. Collection is now ~152 papers. Not exhaustive — treat as a scaffold to keep extending as you read. See `NOTES.md` for what's thin and worth digging into next.
